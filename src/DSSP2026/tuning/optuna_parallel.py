...
def plot_optuna_parallel_coordinates(
    ...
    *,
    ...
    # 2. Axis set: requested params, else auto-inferred from the study —
    #    every param present on *all* selected trials (so the plot works for any
    #    study: MLP, XGBoost, etc., not just a hardcoded MLP list). Conditional
    #    params (e.g. MLP width2/width3, present only for some trials) are
    #    naturally excluded because a parallel-coordinates line needs a value on
    #    every axis. `drop` prunes further.
    if params is not None:
        axes = list(params)
    else:
        common = set(trials[0].params)
        for t in trials[1:]:
            common &= set(t.params)
    ...
    axes = [p for p in axes if all(p in t.params for t in trials)]
    if not axes:
