...
Typical notebook use::
    ...
    from DSSP2026.workflows.databricks import launch_dashboard

    url = launch_dashboard(study_db="/dbfs/FileStore/dssp/study.db")
    # -> prints and returns the proxy URL; click it.

To stop it later::

    from DSSP2026.workflows.databricks import stop_dashboard
    stop_dashboard()
"""
...
def _resolve_db(study_db) -> str:
    """Resolve and validate the study DB path; return it as a string.

    Accepts a path or None. None falls back to config.STUDY_DB (which honours
    DSSP_STUDY_DB). On Databricks the DB usually lives on DBFS; the local FUSE
    ...
    if not Path(s).exists():
        raise FileNotFoundError(
            ...
            "(python -m DSSP2026.workflows.cli run ...) to produce a study.db, "
            "or pass study_db=<path> pointing at an existing one. On DBFS use "
            "the /dbfs/... FUSE path (e.g. /dbfs/FileStore/dssp/study.db).")
    return s


def launch_dashboard(study_db=None, *, port: int = 8501,
                     wait: bool = True, timeout: float = 30.0) -> str:
    """Start the Streamlit dashboard on the driver and return its proxy URL.

    Parameters
    ----------
    study_db : str | Path | None
        Path to the study database the dashboard reads. None -> config.STUDY_DB
        (honours the DSSP_STUDY_DB env var). DBFS paths should be the /dbfs/...
