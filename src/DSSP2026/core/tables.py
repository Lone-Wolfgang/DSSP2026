...
def save_generic_table_png(
    ...
    *,
    ...
    col_widths_chars = [
        max(len(str(col)), *(len(str(v)) for v in display[col].values)) + 3
        for col in cols
    ]
...
def style_tree_metrics_table(metrics_df: pd.DataFrame, *, context: str = "report"):
    """Styler for the tree depth/metrics table."""
...
# ---------------------------------------------------------------------------
# Classification tables (shared by every classifier family)
# ---------------------------------------------------------------------------

def style_classification_report(report_df: pd.DataFrame, *, context: str = "report"):
    """Styler for a per-class precision/recall/F1/support report.

    Expects columns: Class, Precision, Recall, F1, Support (as produced by
    core.metrics.make_classification_report_df). Summary rows (accuracy,
    macro avg, weighted avg) are visually separated.
    """
...
def save_classification_report_png(report_df: pd.DataFrame, path, *,
    ...
    """Render the classification report as a PNG table."""
    fmt = {
...
def style_classification_comparison(comparison_df: pd.DataFrame, *,
    ...
    """Styler for a classifier comparison table; best row (max best_by) highlighted.

    Expects a 'Model' column plus any of Accuracy / Precision / Recall / F1 /
    ROC-AUC. Higher is better for all of these, so the best row maximizes
    `best_by`.
    """
...
def save_classification_comparison_png(comparison_df: pd.DataFrame, path, *,
                                       best_by: str = "F1", context: str = "report",
                                       dpi: int = 220) -> str:
    """Render the classifier comparison table as a PNG, highlighting the best row."""
    metric_cols = [c for c in comparison_df.columns if c != "Model"]
    if best_by not in comparison_df.columns:
        best_by = metric_cols[-1] if metric_cols else None
