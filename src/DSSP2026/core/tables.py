...
def style_classification_comparison(comparison_df: pd.DataFrame, *,
    ...
    # Only format genuinely numeric columns — string columns like "Feature set"
    # must be excluded or "{:,.4f}".format("all") raises ValueError.
    non_label_cols = [c for c in comparison_df.columns if c != "Model"]
    metric_cols = [c for c in non_label_cols
                   if pd.api.types.is_numeric_dtype(comparison_df[c])]
    ...
    fmt = {c: _safe_fmt for c in metric_cols}
