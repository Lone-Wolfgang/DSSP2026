...
class FitMixin:
    ...
    def best_fit(self, models: Optional[Sequence[str]] = None,
        ...
        # Score the winner on test: prefer persisted test predictions (apply the
        # OOF-tuned thresholds to them); fall back to predicting via the fitted
        # PolicyModel on the test parquet.
        win_members = ([m for m in pool if m != ENSEMBLE_NAME]
                       if win_name == ENSEMBLE_NAME else [win_name])
        cached = self._read_test_predictions(win_name, win_members)
        if cached is not None:
            co, y_true, test_proba = cached
            y_pred = decisions_under_policy(win_policy, co, test_proba, win_thr)
            m = metrics_from_decisions(y_true, y_pred, co)
            model.test_metrics = {k: round(float(v), 4) for k, v in m.items()}
        else:
            test_loaded = self.load_test_data()
