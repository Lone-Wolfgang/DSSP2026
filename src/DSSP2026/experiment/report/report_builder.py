...
report.db schema (one row per study = per best model under ``model_id``):
  ...
  trials(trial_id, model_id, trial_number, params, cv_value, state, rank,
...
def _insert_trials(report, model_id, trial_rows):
    rows = [
        (model_id, t["trial_number"], json.dumps(t["params"]), t["cv_value"],
         t["state"], t["rank"], int(bool(t["is_best"])), t["duration_sec"])
        for t in trial_rows
    ]
    report.executemany(
        "INSERT INTO trials (model_id, trial_number, params, cv_value, state, "
        "rank, is_best, duration_sec) VALUES (?,?,?,?,?,?,?,?)", rows)
